...
from kaggle_toolbox.iter.planners.subset import FixedSubsetIterPlanner, FixedSubsetIterPlannerBuilder, \
...
class TestFixedSubsetIterPlanner:
    ...
    def test_fixed_subset_iter_planner_nat_do_end_of_epoch_stop(self):
    ...
    def test_fixed_subset_iter_planner_nat_no_end_of_epoch_stop(self):
        data_loader = DataLoader(IntDataset(9), batch_size=2, num_workers=2)
        planner: IterPlanner[torch.Tensor] = FixedSubsetIterPlanner(
    ...
    def test_fixed_subset_iter_planner_frac_do_end_of_epoch_stop(self):
        data_loader = DataLoader(IntDataset(9), batch_size=2, num_workers=2)
    ...
    def test_fixed_subset_iter_planner_frac_no_end_of_epoch_stop(self):
        data_loader = DataLoader(IntDataset(9), batch_size=2, num_workers=2)
        planner: IterPlanner[torch.Tensor] = FixedSubsetIterPlannerBuilder(
            subset_size=FracSubsetSize(0.5),
            stop_at_epoch_end=False).build(data_loader=data_loader)
        it = planner.get_next_iter()
        assert len(it) == 2
        assert _unwrap_item(next(it)) == (Index(0, (0, 5)), [0, 1])
        assert _unwrap_item(next(it)) == (Index(0, (1, 5)), [2, 3])
