...
class ChoiceFunction(t_ext.Protocol):

    def __call__(self, x: torch.Tensor, /, dim: int = -1) -> torch.Tensor:
        ...


class BinFunction(t_ext.Protocol):
...
class ODST(ModuleWithInit):
    def __init__(
            ...
            initialize_response_: Initializer = nn.init.normal_,
            initialize_selection_logits_: Initializer = nn.init.uniform_,
        ...
        self.response = torch.nn.parameter.Parameter(
            torch.zeros([num_trees, tree_output_dim, 2**depth]),
            requires_grad=True)
        initialize_response_(self.response)

        self.feature_selection_logits = torch.nn.parameter.Parameter(
            torch.zeros([in_features, num_trees, depth]),
            requires_grad=True)
        initialize_selection_logits_(self.feature_selection_logits)

        self.feature_thresholds = torch.nn.parameter.Parameter(
            torch.full([num_trees, depth], float("nan"), dtype=torch.float32),
            requires_grad=True,
        )  # nan values will be initialized on first batch (data-aware init)
        self.log_temperatures = torch.nn.parameter.Parameter(
            torch.full([num_trees, depth], float("nan"), dtype=torch.float32),
            requires_grad=True,
        )
        ...
        with torch.no_grad():
            ...
            self.bin_codes_1hot = torch.nn.parameter.Parameter(bin_codes_1hot, requires_grad=False)
            # ^-- [depth, 2 ** depth, 2]
    ...
    def initialize(self, input, eps=1e-6):
        ...
        with torch.no_grad():
            ...
            self.feature_thresholds.data[...] = torch.as_tensor(  # type: ignore
                list(
                    map(
                        np.percentile,
                        _check_numpy(feature_values.flatten(1, 2).t()),
            ...
            temperatures = np.percentile(
                _check_numpy(abs(feature_values - self.feature_thresholds)),
            ...
            self.log_temperatures.data[...] = torch.log(torch.as_tensor(temperatures) + eps)  # type: ignore
